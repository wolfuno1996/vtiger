...
function dataIsEmpty( data )
{
	...
	if ( !value || value == '<br>' || value == '&nbsp;' || value == ' ' || value == '&nbsp;<br>' || value == ' <br>' )
		return true;

	var otherTags = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'div'];
	var tag = $(value);
	var tagName = tag.prop('tagName').toLowerCase();
	var startTag = tag.startTag();
	var endTag = tag.endTag();
	for (var i = 0; i < otherTags.length; i++) {
		var t = otherTags[i];

		if ($.inArray(t, otherTags)
			&& (value == (startTag + '&nbsp;<br>' + endTag) || (value == startTag + '<br>' + endTag) || (value == startTag + '&nbsp;' + endTag))) {
			return true;
		}
	}
...
function addPlaceholder(ev) {
	...
	if (editor.mode == 'wysiwyg')
	{
		...
		if ( dataIsEmpty( root.getHtml() ) )
		{
			...
			var firstElement = root.getFirst();
			firstElement.setHtml(placeholder);
...
function removePlaceholder(ev) {
	...
	if (editor.mode == 'wysiwyg' )
	{
		...
		var firstElement = root.getFirst();
		var tag = $(firstElement.$);
		var tagName = tag.prop('tagName').toLowerCase();
		var startTag = tag.startTag();
		var endTag = tag.endTag();

		// fill it properly
		if (CKEDITOR.dtd[ root.getName() ][tagName])
		{
			root.setHtml( startTag + '<br/>' + endTag );
